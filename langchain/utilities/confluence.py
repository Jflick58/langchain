"""Util that calls Confluence."""
from typing import Any, Dict, List, Optional

from pydantic import BaseModel, Extra, root_validator

from langchain.tools.confluence.prompt import (
    CONFLUENCE_CATCH_ALL_PROMPT,
    CONFLUENCE_PAGE_CREATE_PROMPT,
    CONFLUENCE_CQL_PROMPT,
)
...
class ConfluenceAPIWrapper(BaseModel):
    ...
    operations: List[Dict] = [
        {
            "mode": "cql",
            "name": "CQL Query",
            "description": CONFLUENCE_CQL_PROMPT,
        },
        {
            "mode": "create_page",
            "name": "Create Page",
            "description": CONFLUENCE_PAGE_CREATE_PROMPT,
        },
        {
            "mode": "other",
            "name": "Catch all Confluence API call",
            "description": CONFLUENCE_CATCH_ALL_PROMPT,
        },
    ]
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        confluence_username = get_from_dict_or_env(values, "confluence_username", "CONFLUENCE_USERNAME")
        values["confluence_username"] = confluence_username

        confluence_api_token = get_from_dict_or_env(
            values, "confluence_api_token", "CONFLUENCE_API_TOKEN"
        )
        values["confluence_api_token"] = confluence_api_token

        confluence_instance_url = get_from_dict_or_env(
            values, "confluence_instance_url", "CONFLUENCE_INSTANCE_URL"
        )
        values["confluence_instance_url"] = confluence_instance_url
    ...
    def search(self, query: str) -> str:
        issues = self.confluence.jql(query)
    ...
    def project(self) -> str:
        projects = self.confluence.projects()
    ...
    def create(self, query: str) -> str:
        ...
        return self.confluence.issue_create(fields=dict(params))
